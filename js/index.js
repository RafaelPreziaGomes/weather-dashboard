...
document
  .querySelector("#clearCityButton")
  .addEventListener("click", function () {
    document
      .getElementById("searchHistory")
      .parentNode.removeChild(document.getElementById("searchHistory"));
  });
...
function getGeo(cityValue) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${cityValue}&appid=ccacfb31397f03e28f037bc9028878d9`
  )
...
function getWeather(lat, lon) {
  fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&appid=ccacfb31397f03e28f037bc9028878d9&units=imperial`
  )
    .then((reponse) => reponse.json())
    .then((data) => {
      console.log(data);
